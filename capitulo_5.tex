...
\begin{figure}[H]
	...
	\caption{Gráfica de error para guitarra en arquitectura 1}
	\label{fig:arq1_guitar_error}
\end{figure}

En la Tabla \ref{tabla:val_error_gui_arq1} se puede observar que la tendencia del error es a la baja, llegando a valores de 1.3161 en la fase de entrenamiento y de 4.1383 en la fase de validación, y aunque por la época 100 pareciera que el error va a tender a subir, no es así al final.
...
\begin{figure}[H]
	...
	\caption{Gráfica de aprendizaje para guitarra en arquitectura 1}
	\label{fig:arq1_guitar_acc}
\end{figure}

En la Tabla \ref{tabla:val_acc_gui_arq1} se observa que para la fase de entrenamiento se llegó a un valor final de 56.66\% y en la fase de validación a un 58.18\%, este porcentaje podría parecer bajo, sin embargo se tuvieron máximos en algunas épocas de hasta 72.26\% y 58.18\% respectivamente.
...
\begin{figure}[H]
	...
	\caption{Gráfica de error para bajo en arquitectura 1}
	\label{fig:arq1_bass_error}
\end{figure}

En la Tabla \ref{tabla:val_error_bass_arq1} se puede observar que el error llegó a valores de 2.3281 en la fase de entrenamiento y de 2.8547 en la fase de validación. 
...
En la Tabla \ref{tabla:val_acc_bass_arq1} se observa que para la fase de entrenamiento se llegó a un valor final de 74.08\% y en la fase de validación a un 52.28\%.
...
En la Figura \ref{fig:arq2_guitar_error} se muestra como fue disminuyendo el error usando esta arquitectura para  guitarra. Se puede observar que en la fase de validación el error disminuyo más lentamente que en la fase de entrenamiento.

\begin{figure}[H]
	\centerline{\includegraphics[width=12cm]{arq2_guitar_error.png}}
	\caption{Gráfica de error para guitarra en arquitectura 2}
...
En la Tabla \ref{tabla:val_acc_gui_arq2} se observa que para la fase de entrenamiento se llegó a un valor final de 55.58\% y en la fase de validación a un 42.73\%, obteniendo valores máximos en algunas épocas de hasta 65.55\% y 42.73\% respectivamente.
...
En la Figura \ref{fig:arq2_bass_error} se muestra como fue disminuyendo el error usando esta arquitectura para  bajo. El error desciende de una forma más lineal que en el caso de la guitarra.
...
En la Tabla \ref{tabla:val_acc_bass_arq2} se observa que para la fase de entrenamiento se llegó a un valor final de 61.70\% y en la fase de validación a un 29.88\%, obteniendo valores máximos en algunas épocas de hasta 64.26\% y 36.83\% respectivamente.
...
En la Figura \ref{fig:arq3_guitar_error} se muestra como fue cambiando el error usando esta arquitectura para  guitarra. Se puede observar que el error va creciendo conforme las épocas pasan.
...
En la Tabla \ref{tabla:val_acc_gui_arq3} se observa que para la fase de entrenamiento se llegó a un valor final de 17.02\% y en la fase de validación a un 9.69\%, obteniendo máximos de hasta 19.64\% y 13.28\% respectivamente.
...
Esta gráfica muestra que el bajo va aprendiendo pero la tasa de aprendizaje es bastante baja.
...
En la Tabla \ref{tabla:val_acc_bass_arq3} se observa que para la fase de entrenamiento se llegó a un valor final de 15.07\% y en la fase de validación a un 12.97\%, obteniendo máximos de hasta 17.34\% y 13.68\% respectivamente.
...
En esta etapa se evaluara la salida de las redes desde un concepto musical. Se verá como están interactuando las notas en una salida de nuestra red.

Se tomaran muestras de pocos compases de las salidas de nuestras redes, esto para ser más objetivos con la interpretación de los resultados.

Cada red fue alimentada con las notas de una canción base, y con esta entrada más lo que la red pudo aprender en el proceso de entrenamiento generara una salida.

\subsection{Arquitectura 1}

Para la interpretación de resultados, en esta red se usó una canción base con tonalidad de Mi Mayor, y se espera que la salida posea esta misma tonalidad.
...
La nueva pieza musical posee la tonalidad de Do Mayor, tal como las secuencias de entradas, lo que nos indica que esta red está trabajando correctamente.

La salida que estamos obteniendo con esta red usa principalmente notas de corcheas y negras, lo cual corresponde muy bien con las notas de entrada que está recibiendo la red.
...
En base a estos argumentos podemos verificar que esta red está cumpliendo su cometido de aprender secuencias y generar salidas que tengan sentido con las secuencias de entrada.


\subsection{Arquitectura 3}

Para la interpretación de resultados, en esta red se usó una canción base con tonalidad de Sol Mayor, y se espera que la salida posea esta misma tonalidad.
...
En las figuras \ref{fig:arq1_guitar_val} y \ref{fig:arq1_bass_val} se puede observar que conforme van pasando las épocas, el número de canciones creadas que poseen la misma tonalidad que la canción base va en aumento.

En la tabla \ref{tabla:val_arq1} vemos que los valores alcanzados por esta arquitectura son de 64.26\% para guitarra y de 65.38\% para bajo, teniendo unos máximos de 64.26\% y 67.50\% respectivamente.
...
En las figuras \ref{fig:arq2_guitar_val} y \ref{fig:arq2_bass_val} se puede observar que conforme van pasando las épocas, el número de canciones creadas que poseen la misma tonalidad que la canción base va en aumento, sin embargo el crecimiento en las canciones de bajo es mucho más lento en comparación a la guitarra.

En la tabla \ref{tabla:val_arq2} vemos que los valores alcanzados por esta arquitectura son de 49.81\% para guitarra y de 36.07\% para bajo, teniendo unos máximos de 49.81\% y 37.62\% respectivamente.
...
En las figuras \ref{fig:arq3_guitar_val} y \ref{fig:arq3_bass_val} se puede observar que conforme van pasando las épocas, el número de canciones creadas que poseen la misma tonalidad que la canción base va en aumento, sin embargo el porcentaje final es bastante bajo después de 200 épocas.

En la tabla \ref{tabla:val_arq3} vemos que los valores alcanzados por esta arquitectura son de 10.94\% para guitarra y de 11.85\% para bajo, teniendo unos máximos de 10.94\% y 11.88\% respectivamente.
